...
local function DoDamage(inst, targets, skiptoss)
    ...
    if inst.AnimState ~= nil then
        ...
        inst:Show()
        inst:DoTaskInTime(--[[inst.AnimState:GetCurrentAnimationLength()]]1 + 2 * FRAMES, inst.Remove)
...
local function FastForwardTrail(inst, pct)
    ...
    local len = 1 --inst.AnimState:GetCurrentAnimationLength()
...
local function trailfn()
    ...
    inst._task = inst:DoTaskInTime(--[[inst.AnimState:GetCurrentAnimationLength()]]1 + 2 * FRAMES, inst.Remove)

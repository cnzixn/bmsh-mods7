...
local function MakeAnySpecificBlueprint(specific_item)
	local ctor = function()
	    ...
	    for k,v in pairs(GetAllRecipes()) do --改，这是手机的
...
local prefabs = {}
table.insert(prefabs,Prefab('living_artifact_blueprint',MakeAnySpecificBlueprint('living_artifact'),assets)) --钢铁战衣
table.insert(prefabs,Prefab('bundle_blueprint',MakeAnySpecificBlueprint('bundle'),assets)) --打包纸
table.insert(prefabs,Prefab('armorvortexcloak_blueprint',MakeAnySpecificBlueprint('armorvortexcloak'),assets)) --暗影斗篷

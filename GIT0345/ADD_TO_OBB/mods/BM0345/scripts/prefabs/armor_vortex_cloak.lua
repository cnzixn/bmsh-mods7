...
Asset( "ATLAS", "images/inventoryimages/armor_vortex_cloak.xml"),
}
...
local function autoload(inst)

    if inst.components.equippable:IsEquipped() and inst.components.container:Has("nightmarefuel",1) then
	   local t1 = math.floor((inst.components.armor.maxcondition - inst.components.armor.condition)/inst.val)
	   local num = 0

	     for k,v in pairs(inst.components.container.slots) do
              if v.prefab == "nightmarefuel" and v.components.stackable then
			     for k = 1, v.components.stackable.stacksize do
					 num = num +1
				 end
			 end
        end
	  ...
	  if t1 >= 1 and num >= t1 then
        for i=1,t1 do
		   inst.components.container:ConsumeByName("nightmarefuel",1)
		   inst.components.armor.condition = inst.components.armor.condition + inst.val
	 ...
	 if t1 >= 1 and num < t1 then
	   ...
	   print(inst.components.armor.condition,inst.components.armor.maxcondition,t1)
    end
...
local function onequip(inst, owner) 
  ...
  --  inst:ListenForEvent("attacked", inst.OnBlocked, owner)      -- function(inst,data) OnBlocked(owner,inst) end
	...
	inst.loadtask = inst:DoPeriodicTask(0.1,function()  autoload(inst) end) 
...
local function ontakefuel(inst)
    ...
    inst.components.armor:SetCondition( math.min( inst.components.armor.condition + inst.val , inst.components.armor.maxcondition) )
    GetPlayer().components.sanity:DoDelta(-TUNING.SANITY_TINY)
...
local function fn()
   ...
   inst.components.inventoryitem.imagename = "armor_vortex_cloak"
   	inst.components.inventoryitem.atlasname = "images/inventoryimages/armor_vortex_cloak.xml"
    ...
    inst.components.armor:InitCondition(150*3, 1) --(TUNING.ARMORVORTEX, TUNING.ARMORVORTEX_ABSORPTION)
	...
	inst.val = inst.components.armor.maxcondition/20
...
local function vortex_cloak_fx()
	local inst = CreateEntity()
	inst.entity:AddTransform()
	inst.entity:AddAnimState()
	inst.entity:AddSoundEmitter()
...
return Prefab( "common/inventory/armorvortexcloak", fn, assets),
        ...
        Prefab( "common/inventory/vortex_cloak_fx", vortex_cloak_fx, assets) 

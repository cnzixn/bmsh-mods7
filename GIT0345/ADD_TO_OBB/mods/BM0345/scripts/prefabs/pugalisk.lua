...
{	
    ...
    Asset( "ATLAS", 'images/inventoryimages/pugalisk.xml'),
}
...
local function redirecthealth(inst, amount, overtime, cause, ignore_invincible)
    ...
    if amount < 0 and( (inst.components.segmented and inst.components.segmented.vulnerablesegments == 0) or inst:HasTag("tail") or inst:HasTag("head") ) then
        ...
        if cause == GetPlayer().prefab then
            GetPlayer().components.talker:Say("这里的皮太硬了！打不动！") --原 GetPlayer().components.talker:Say(GetString(GetPlayer().prefab, "ANNOUNCE_PUGALISK_INVULNERABLE"))        
        end
...
local function segmentfn(Sim) --躯干(拱形)
...
local function bodyfn(Sim) --挨着地面那节
    ...
    inst:ListenForEvent("bodycomplete", function() 
                if inst.exitpt then
                    ...
                    inst.SoundEmitter:PlaySound("dontstarve_DLC001/creatures/bearger/groundpound") --钻入地面的音效
...
--[[    
    ...
    inst:AddComponent("inspectable")
    inst.components.inspectable.nameoverride = "pugalisk"
    inst.name = "狮鼻蛇怪" --STRINGS.NAMES.PUGALISK
...
local function corpsefn(Sim) --死后的洞
...
return  Prefab( "common/monsters/pugalisk", fn, assets, prefabs),
        ...
        Prefab( "common/monsters/pugalisk_tail", tailfn, assets, prefabs), --尾巴

local assets =
{
	...
	Asset("ANIM", "anim/meat_rack_food2.zip"), --改了个名，为了避免动画重名
...
local function onstartdrying(inst, build)
	...
	if not inst:HasTag("burnt") then
		...
		inst.AnimState:PushAnimation("drying_loop", true)
	    inst.AnimState:OverrideSymbol("swap_dried", "meat_rack_food2", build)
...
local function setdone(inst, product)
	if not inst:HasTag("burnt") then
	    ...
	    inst.AnimState:OverrideSymbol("swap_dried", "meat_rack_food2", product)
	end
end

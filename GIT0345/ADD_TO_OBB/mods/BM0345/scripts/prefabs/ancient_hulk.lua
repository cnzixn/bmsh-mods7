...
local prefabs =
{
    ...
    "infused_iron",
    "living_artifact_blueprint", --盔甲蓝图
}
    ...
    for i=1,number do        
        ...
        if tile ~= GROUND.IMPASSABLE and tile ~= GROUND.INVALID and not GetWorld().Map:IsWater(tile) then
            GetWorld():DoTaskInTime(math.random()*0.3, function()            
                ...
                inst.SoundEmitter:PlaySound("Hamlet/creatures/boss/hulk_metal_robot/rock")
...
local function OnHit(inst, dist)    
    ...
    inst.SoundEmitter:PlaySound("Hamlet/creatures/enemy/metal_robot/ribs/step_wires")
    inst.AnimState:PushAnimation("open")
    --inst.SoundEmitter:PlaySound("dontstarve_DLC003/creatures/boss/hulk_metal_robot/rust")    
    inst.SoundEmitter:PlaySound("Hamlet/creatures/boss/hulk_metal_robot/rust")
...
local function orbsmallfn(Sim)
    ...
    inst.Physics:CollidesWith(COLLISION.INTWALL) --估计用手柄这里会崩

...
local function NormalRetargetFn(inst)
    return FindEntity(inst, 16, --TUNING.ANTMAN_WARRIOR_TARGET_DIST,
        function(guy)
            if guy.components.health and not guy.components.health:IsDead() and inst.components.combat:CanTarget(guy) then
                ...
                if guy:HasTag("player") and guy.components.inventory and guy:GetDistanceSqToInst(inst) < --[[TUNING.ANTMAN_WARRIOR_ATTACK_ON_SIGHT_DIST*TUNING.ANTMAN_WARRIOR_ATTACK_ON_SIGHT_DIST]]8*8 and not guy:HasTag("ant_disguise") then return guy end
            end
        end
    )
end
...
local function TransformToNormal(inst)
    ...
    --ReplaceEntity(inst, normal) --改，禁用

    inst:Remove()
end

local function oninit(inst)
  local pisoprair = GetClosestInstWithTag("canbuild", inst, 30)
  if pisoprair then 
    local x, y, z = pisoprair.Transform:GetWorldPosition()
...
local function SetNormalAnt(inst)
    ...
    inst.components.combat:SetDefaultDamage(42) --(TUNING.ANTMAN_WARRIOR_DAMAGE)
    inst.components.combat:SetAttackPeriod(3) --(TUNING.ANTMAN_WARRIOR_ATTACK_PERIOD)
    inst.components.combat:SetKeepTargetFunction(NormalKeepTargetFn)
    
    inst.components.locomotor.runspeed = 7 --TUNING.ANTMAN_WARRIOR_RUN_SPEED
    inst.components.locomotor.walkspeed = 3.5 --TUNING.ANTMAN_WARRIOR_WALK_SPEED
    ...
    inst.components.health:SetMaxHealth(300) --(TUNING.ANTMAN_WARRIOR_HEALTH)
...
local function common()
    ...
    inst.components.locomotor.runspeed = 7 --TUNING.ANTMAN_WARRIOR_RUN_SPEED --5
    inst.components.locomotor.walkspeed = 3.5 --TUNING.ANTMAN_WARRIOR_WALK_SPEED --3
    ...
    inst.components.named.possiblenames = STRINGS.PIGNAMES --STRINGS.ANTWARRIORNAMES
...
--[[
    ...
    inst:ListenForEvent("antqueenbattle", function() inst.SetAporkalypse(true) end, GetWorld())

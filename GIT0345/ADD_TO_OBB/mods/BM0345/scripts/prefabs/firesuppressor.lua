...
--由于官方改了complexprojectile组件，灭火器也跟着改了。
--我也只好跟着改了……真是让人头秃 (≖_≖ )
local assets = 
{
...
--点击灭火器范围显示
	...
	local show_range = TheSim:FindEntities(pos.x,pos.y,pos.z, 2, {"show_range"} )
	if #show_range < 1 then
	local range = SpawnPrefab("show_range")
...
local function fn()
 ...
 table.insert(inst.components.firedetector.NOTAGS, "campfire") --改，不灭营火
...
require "prefabutil"
return Prefab( "common/show_range", range_fn, assets),
Prefab("firesuppressor", fn, assets, prefabs), 

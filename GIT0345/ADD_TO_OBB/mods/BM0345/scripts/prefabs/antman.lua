    ...
    return FindEntity(inst, TUNING.PIG_TARGET_DIST,
        function(guy)
            if guy.components.health and not guy.components.health:IsDead() and inst.components.combat:CanTarget(guy) and not is_complete_disguise(guy) then
                ...
                if guy:HasTag("player") and guy.components.inventory and guy:GetDistanceSqToInst(inst) < --[[TUNING.ANTMAN_ATTACK_ON_SIGHT_DIST*TUNING.ANTMAN_ATTACK_ON_SIGHT_DIST]]4*4 and not guy:HasTag("ant_disguise") then return guy end
            end
        end)
end
...
local function TransformToWarrior(inst, from_limbo_or_asleep)
    if from_limbo_or_asleep then
		...
		--ReplaceEntity(inst, warrior) --改，禁用
        inst:Remove()
    else
...
local function SetNormalAnt(inst)
    ...
    inst.components.combat:SetDefaultDamage(22.5) --(TUNING.ANTMAN_DAMAGE)
    inst.components.combat:SetAttackPeriod(3) --(TUNING.ANTMAN_ATTACK_PERIOD)
    inst.components.combat:SetKeepTargetFunction(NormalKeepTargetFn)
    inst.components.locomotor.runspeed = 5 --TUNING.ANTMAN_RUN_SPEED
    inst.components.locomotor.walkspeed = 3 --TUNING.ANTMAN_WALK_SPEED
    ...
    inst.components.health:SetMaxHealth(250) --(TUNING.ANTMAN_HEALTH)
...
local function common()
    ...
    inst.components.locomotor.runspeed = 5 --TUNING.ANTMAN_RUN_SPEED --5
    inst.components.locomotor.walkspeed = 3 --TUNING.ANTMAN_WALK_SPEED --3
    ...
    inst.components.named.possiblenames = STRINGS.PIGNAMES --STRINGS.ANTNAMES
    inst.components.named:PickNewName()
	
    ------------------------------------------
    inst:AddComponent("follower")
    inst.components.follower.maxfollowtime = 2.5*30*16 --TUNING.ANTMAN_LOYALTY_MAXTIME
    ------------------------------------------

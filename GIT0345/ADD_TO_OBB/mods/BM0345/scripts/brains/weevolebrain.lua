...
local function EatFoodAction(inst)
    ...
    local target = FindEntity(inst, SEE_FOOD_DIST, function(item) return inst.components.eater:CanEat(item) and item:IsOnValidGround() and not item:GetIsOnWater(item:GetPosition():Get()) end)
    if target then

...
local function IsQuaking(inst)
    return GetWorld().components.quaker:IsQuaking() and not (inst.components.health:GetPercent() <= 0.3)
end
...
local function TrySanityAttack(inst)
    return inst.components.health:GetPercent() <= 0.75 and inst.sanity_attack_count < inst.max_sanity_attack_count and CanAttack(inst) and not GetWorld().components.quaker:IsQuaking()
end
